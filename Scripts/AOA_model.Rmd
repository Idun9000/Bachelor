...
```{r}
# Performing K-fold
cv_results <- kfold(AOA_model_for_kfold, K = 5)
...
posterior_predictive <- posterior_predict(AOA_model_for_accuracy, newdata = merged_AOA_test) # matrix where each row corresponds to a prediction for an observation in the test set, and each column corresponds to a draw from the posterior distribution of the predicted probabilities

# for each observation, calculate the mean of all the draws from the posterior predictive distribution for that observation.
merged_AOA_test$predicted_prob <- apply(posterior_predictive, 2, mean)

# If the predicted probability is greater than 0.5, we predict the class as 1 (knows the word)
merged_AOA_test$predicted_class <- ifelse(merged_AOA_test$predicted_prob > 0.5, 1, 0)

# Calculate accuracy - compare predictions with actual outcome
accuracy <- mean(merged_AOA_test$predicted_class == merged_AOA_test$Knows_Word)
